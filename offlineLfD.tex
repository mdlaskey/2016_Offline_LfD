...
In the supervised learning approach to LfD, \adnote{the robot learns a policy -- a mapping from the state space to controls -- } via regression or classification. The \adnote{standard} approach to LfD is to \adnote{let the supervisor provide multiple demonstrations of performing the task, which induces a set of state-control examples for} the states the supervisor is likely to visit~\cite{pomerleau1989alvinn}. We \adnote{refer to} this as \emph{Human-Centric} (HC) \adnote{sampling}, because the human controls what states the robot will observe. \adnote{Because of} error in learning, the robot's policy will not \adnote{perfectly }match the supervisor\adnote{, and will end up visiting new} states when performing the task. \adnote{An alternative approach to HC sampling} is to have the supervisor retro-actively provide \adnote{corrections to the robot \cite{ross2010efficient,ross2010reduction,laskeyrobot,laskeyshiv,he2012imitation}, i.e. iteratively re-label the states that the robot visits with its current learned policy.}
...
\adnote{Our insight is that} while RC has great advantages, it comes at a cost \adnote{in practice}. One cost is the challenge posed to a \emph{human} supervisor.  RC methods have a human provide retro-active feedback to the robot without observing the effect of their suggested control. \adnote{Instead, the robot moves on to the next state where it asks for a label.}  The human supervisor has to predict how the world will change given a control, instead of \adnote{operating the robot with a feedback loop. This makes labels much more noisy (Fig. \ref{fig:teaser}(b)).}

\adnote{A second cost is that the data set obtained by getting labels on states that occur} due to previous errors \adnote{corresponds to a more complex policy than standard data set obtained through regular demonstrations. In particular, the supervisor has to show the robot complex recovery maneuvers that do not occur during normal task executing. Imagine, for instance, the task of singulating an object from a set of objects on a table (Fig. \ref{fig:teaser}. Standard demonstrations involve moving forward and from side to side to push obstacles out of the way and recover the desired object. But if the robot makes errors and ends up in a failure state (Fig. \ref{fig:teaser}(c,right)), then more complex maneuvers will be required. }
%can cause the human to provide more complex recovery behavior examples. This can be challenging for both the robot's policy to learn and for a human to provide demonstrations at. 
...
We explore the value of the extra overhead of RC \adnote{in particular }for emerging classes of highly-expressive deep learning models such as deep decision trees and neural networks. \adnote{These learners} can more accurately learn the supervisor's policy compared to standard learners (like linear models). 

We  conjecture\adnote{isn't this fact? why conjecture?} that their added expressiveness can help alleviate model error~\cite{vapnik1992principles}, which means the robot may now recover the expected supervisor's policy with enough data. In this case, the robot will \adnote{eventually} converge to the  supervisor's distribution \adnote{with HC sampling}. Thus,  it could be beneficial to collect data from the distribution it is converging towards rather than its current. 
